...
public class Task08
{
	...
	public static void main(String args[])
	{
		...
		String person = "http://data.org#Person";
		OntClass person1 = model1.getOntClass(person);
		OntClass person2 = model2.getOntClass(person);
		ExtendedIterator it1 = person1.listInstances();

		while(it1.hasNext()) {
			Individual ind1 = (Individual) it1.next();
			if( ind1.getProperty(VCARD.Family) == null ){
				Statement st = model2.getIndividual(ind1.toString()).getProperty(VCARD.Family);
				if (st!=null) {
					System.out.println("Propiedad VCARD añadida para: " + ind1.toString());
					ind1.addProperty(VCARD.Family, st.getObject());
				}
			}
			if( ind1.getProperty(VCARD.EMAIL) == null ){
				Statement st = model2.getIndividual(ind1.toString()).getProperty(VCARD.Family);
				if (st!=null) {
					System.out.println("Propiedad EMAIL añadida para: " + ind1.toString());
					ind1.addProperty(VCARD.EMAIL, st.getObject());
				}
			}
			if( ind1.getProperty(VCARD.Given) == null ){
				Statement st = model2.getIndividual(ind1.toString()).getProperty(VCARD.Family);
				if (st!=null) {
					System.out.println("Propiedad Given añadida para: " + ind1.toString());
					ind1.addProperty(VCARD.Given, st.getObject());
				}
			}
