...
public class Task08
{
	...
	public static void main(String args[])
	{
		...
		ExtendedIterator<Individual> persons = model1.listIndividuals(model1.getOntClass("http://data.org#Person"));
		model2.write(System.out, "RDF/XML-ABBREV");
		Property[] lista = {VCARD.Given,VCARD.Family,VCARD.EMAIL,VCARD.FN};
		while(persons.hasNext()) {
			Individual node = persons.next();
			for(Property a:lista) {
				if(null == node.getProperty(a)) {
					Statement c = model2.getIndividual(node.toString()).getProperty(a);
					if(c != null)
						node.addProperty(a, c.getObject());
				}
			}
		}
	}
